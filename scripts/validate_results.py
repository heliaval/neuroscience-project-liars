...
This is the shape-drift gate: both the fixture and the eventual real
...
# sections only -- real transcribed per-participant/per-dyad scores (exp1, exp3,
# exp4, gate, frozen) are copied verbatim from the pipeline's real output and can
# legitimately fall outside it (e.g. a single participant's personalized-model
# AUROC on a 161-trial test fold). See PROGRESS.md for the divergence note this
# resolved. "delta"-keyed subtrees hold paired differences, not AUROC values, and
# are exempt from the AUROC bound regardless of section.
PLACEHOLDER_PATH_PREFIXES = (
    "$.experiments.exp2", "$.experiments.exp5", "$.experiments.exp6",
    "$.experiments.exp7", "$.experiments.exp8",
