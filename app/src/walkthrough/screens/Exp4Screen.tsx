    ...
    <WalkthroughScreen
      ...
      caveat={
        <>
          <Caveat>
            ...
            {test.ci95.lower?.toFixed(4)}, {test.ci95.upper?.toFixed(4)}], which excludes zero on the negative
            side, while the sign test over the same {test.n} pairs gives p {test.sign_test_p.toFixed(4)}, which
            does not reach significance. The permutation p is {test.permutation_p.toFixed(4)}. A combined
            relationship-plus-population model does not fix it either: the net familiarity index has a median
            of {nfi.median_delta.toFixed(4)} with {nfi.n_positive} of {nfi.n} pairs positive.
          </Caveat>
      ...
      secondBeat={
        <>
          ...
          <p className="mt-3 max-w-[60ch] font-serif text-[1.05rem] leading-[1.6] text-ink-soft">
          ...
          <ConditionComparison selected="dyad_specific" />
        </>
      }
    />
  );
